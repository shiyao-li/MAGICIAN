import os
import sys
from macarons.utility.tsp_utils import generate_key_value_splited_dict, line_segment_mesh_intersection, generate_Dijkstra_path
import gc
import shutil
...
def compute_fish_trajectory(params, macarons, camera, gt_scene, surface_scene, 
    ...
    reverse_mapping = {}
    for key, value in splited_pose_space_idx.items():
        tensor_repr = tuple(value[0].cpu().numpy())  # Convert tensor to tuple for hashing
        if tensor_repr not in reverse_mapping:
            reverse_mapping[tensor_repr] = []
        reverse_mapping[tensor_repr].append(key)
    ...
    while pose_i <= params.n_poses_in_trajectory:
        ...
        if (path_record + 1 > len(Dijkstra_path)):
            max_resample_attempts = 5  # 最大重采样次数
            resample_count = 0
            
            while resample_count < max_resample_attempts:
                sampled_poses_params_list = camera.sample_valid_poses_in_space(mesh, proxy_scene, num_samples=300)
                sampled_poses_params_list_with_coverage = []
                
                for poses_params in sampled_poses_params_list:
                    with torch.no_grad():
                        _, _, _, sampled_coverage_gain = predict_coverage_gain_for_single_camera(
                                ...
                                camera=camera, X_cam_world=poses_params[1], fov_camera=poses_params[2])
                    poses_params.append(sampled_coverage_gain.item())
                    sampled_poses_params_list_with_coverage.append(poses_params)
                
                sorted_poses_params_list = sorted(sampled_poses_params_list_with_coverage, key=lambda x: x[-1], reverse=True)
                best_sampled_pose = sorted_poses_params_list[0]

                if torch.equal(current_best_pose[:3], best_sampled_pose[0][:3]) and (path_record + 1 <= len(Dijkstra_path)):
                    continue
                
                # 尝试找到有效的 Dijkstra 路径
                path_found = False
                for best_sampled_pose in sorted_poses_params_list:
                    ...
                    Dijkstra_path = generate_Dijkstra_path(splited_pose_space, path_start_position, path_end_position, intersector, device)
        ...
        for i in range(valid_neighbors.shape[0]):
            # 检查前 prefix_len 个元素是否匹配
            if torch.all(valid_neighbors[i, :3] == torch.tensor(Dijkstra_path[path_record], device=device)):
                matched_indices.append(i)
        import random
        if matched_indices:
            idxx = random.choice(matched_indices)
            next_idx = valid_neighbors[idxx]
        else:
...
def run_fish_test(params_name,
    ...
    lmdb_dir = os.path.join(results_dir, "fish_lmdb")
    ...
    for i in range(len(dataloader.dataset)):
        ...
        for i_scene in range(batch_size):
            ...
            for start_cam_idx_i in range(len(settings.camera.start_positions)):
                ...
                coverage_evolution, X_cam_history, V_cam_history, full_pc, full_pc_colors, full_pc_idx = compute_fish_trajectory(params, macarons,
                                                                                      camera,

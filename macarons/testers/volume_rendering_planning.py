...
def project_points_to_image_plane(points, fov_camera, image_height, image_width):
    """
    将3D点投影到图像平面
    
    Args:
        points: (N, 3) 3D点
        fov_camera: PyTorch3D相机对象
        image_height, image_width: 图像尺寸
    
    Returns:
        valid_pixels: (M, 2) 有效的像素坐标 [y, x]
        valid_mask: (N,) 布尔掩码，表示哪些点在视场内
    """
    if points.shape[0] == 0:
        return torch.zeros(0, 2, device=points.device, dtype=torch.long), torch.zeros(0, device=points.device, dtype=torch.bool)
    
    # 获取视场内的点
    fov_points, fov_mask = get_points_in_fov_manual(points, fov_camera, image_height, image_width)
    
    if fov_points.shape[0] == 0:
        return torch.zeros(0, 2, device=points.device, dtype=torch.long), fov_mask
    
    # 投影到NDC坐标
    ndc_points = fov_camera.get_full_projection_transform().transform_points(fov_points)
    
    # 转换为像素坐标
    # NDC范围 [-1, 1] -> 像素坐标 [0, width-1], [0, height-1]
    pixel_x = ((ndc_points[:, 0] + 1) / 2 * (image_width - 1)).long()
    pixel_y = ((ndc_points[:, 1] + 1) / 2 * (image_height - 1)).long()
    
    # 裁剪到有效范围
    pixel_x = torch.clamp(pixel_x, 0, image_width - 1)
    pixel_y = torch.clamp(pixel_y, 0, image_height - 1)
    
    valid_pixels = torch.stack([pixel_y, pixel_x], dim=1)  # [y, x] 格式
    
    return valid_pixels, fov_mask

def get_points_in_fov_manual(points, fov_camera, image_height, image_width):
    """
    手动实现视场检查，避免依赖camera对象的方法
    """
    if points.shape[0] == 0:
        return points, torch.zeros(0, device=points.device, dtype=torch.bool)
    
    # 转换到相机坐标系
    view_transform = fov_camera.get_world_to_view_transform()
    points_view = view_transform.transform_points(points)
    
    # 投影变换
    projection_transform = fov_camera.get_full_projection_transform()
    points_ndc = projection_transform.transform_points(points)
    
    # 计算视场内掩码
    # 1. 深度检查：点在相机前方
    depth_mask = points_view[:, 2] > 0
    ...
    # 综合掩码
    fov_mask = depth_mask & ndc_mask
    fov_points = points[fov_mask]
    
    return fov_points, fov_mask

def create_pixel_mask(pixel_coords, image_height, image_width):
    """
    从像素坐标创建图像掩码
    ...
    Returns:
        mask: (H, W) 布尔掩码
    """
    mask = torch.zeros(image_height, image_width, device=pixel_coords.device, dtype=torch.bool)
    
    if pixel_coords.shape[0] > 0:
        # 去重像素坐标
        unique_pixels = torch.unique(pixel_coords, dim=0)
        y_coords = unique_pixels[:, 0]
        x_coords = unique_pixels[:, 1]
...
def evaluate_pose_unknown_pixels(pose_idx, camera, params, covered_points, occupancy_points):
    """
    评估单个pose能看到的未知pixels数量（双层渲染法）
    
    Args:
        pose_idx: pose索引
        camera: 相机对象  
        params: 参数对象
        covered_points: (M, 3) 确定的表面点（来自covered_scene）
        occupancy_points: (N, 3) 预测的占用点
    
    Returns:
        unknown_pixel_count: 未知像素数量
        analysis: 详细分析信息
    """
    # 获取pose对应的相机参数
    ...
    # 第1层：渲染确定的表面点（covered_scene的点）
    covered_pixels, covered_fov_mask = project_points_to_image_plane(
        covered_points, fov_camera, image_height, image_width
    )
    covered_mask = create_pixel_mask(covered_pixels, image_height, image_width)
    
    # 第2层：渲染预测的占用点
    occupancy_pixels, occupancy_fov_mask = project_points_to_image_plane(
        occupancy_points, fov_camera, image_height, image_width
    )
    occupancy_mask = create_pixel_mask(occupancy_pixels, image_height, image_width)
    
    # 核心逻辑：未知pixels = 有occupancy预测但没有covered确认
    unknown_mask = occupancy_mask & (~covered_mask)
    unknown_pixel_count = unknown_mask.sum().item()
    
    # 详细分析
    analysis = {
        'pose_idx': pose_idx.cpu().numpy().tolist(),
        'covered_points_in_fov': covered_fov_mask.sum().item(),
        'occupancy_points_in_fov': occupancy_fov_mask.sum().item(),
        'covered_pixels': covered_mask.sum().item(),
        'occupancy_pixels': occupancy_mask.sum().item(),
        'unknown_pixels': unknown_pixel_count,
        'total_pixels': image_height * image_width,
        'unknown_ratio': unknown_pixel_count / (image_height * image_width)
    }
    
    return unknown_pixel_count, analysis

def find_best_pose_by_unknown_pixels(candidate_poses, camera, params, covered_points, occupancy_points):
    """
    找到能看到最多未知pixels的最佳pose
    """
    all_scores = []
    all_analysis = []
    
    for i, pose_idx in enumerate(candidate_poses):
        unknown_pixel_count, analysis = evaluate_pose_unknown_pixels(
            pose_idx, camera, params, covered_points, occupancy_points
        )
        
        all_scores.append(unknown_pixel_count)
        all_analysis.append(analysis)
    
    # 选择得分最高的pose
    best_pose_idx = np.argmax(all_scores)
    
    return best_pose_idx, all_scores, all_analysis
...
def modified_compute_trajectory_with_dual_rendering(
    ...
    for pose_i in range(params.n_poses_in_trajectory + 1):
        ...
        # 双层渲染选择最佳视点
        best_neighbor_idx, all_scores, all_analysis = find_best_pose_by_unknown_pixels(
            valid_neighbors, camera, params, covered_points, occupancy_points
        )
        next_idx = valid_neighbors[best_neighbor_idx]
...
def run_volume_rendering_test(params_name,
             model_name,
             results_json_name,
             numGPU,
             test_scenes,
    ...
    for i in range(len(dataloader.dataset)):
        ...
        for i_scene in range(batch_size):
            ...
            for start_cam_idx_i in range(len(settings.camera.start_positions)):
                ...
                coverage_evolution, X_cam_history, V_cam_history = modified_compute_trajectory_with_dual_rendering(params, macarons,
                                                                                      camera,
